...
export class BlocklistManager extends LitElement {
  ...
  private async refresh(): Promise<void> {
    ...
    try {
      this.videos = await api.get<BlockedVideo[]>(`/api/children/${this.childId}/blocked`);
    } catch (err) {
  ...
  private async onRemove(videoId: string): Promise<void> {
    if (this.childId == null) return;
    try {
      await api.delete(`/api/children/${this.childId}/blocked/${encodeURIComponent(videoId)}`);
      await this.refresh();
    } catch (err) {
      this.error = (err as Error).message;
    }
      ...
      <form @submit=${this.onAdd}>
        ...
        <label for="block-reason">
          ...
          <input
            ...
            type="text"
            .value=${this.reason}
            @input=${(e: Event) => (this.reason = (e.target as HTMLInputElement).value)}
          />
        </label>
                ...
                (v) => html`
                  <li>
                    <div>
                      ...
                      ${v.reason ? html`<div class="empty">— ${v.reason}</div>` : nothing}
                    </div>
                    <button
                      type="button"
                      class="secondary"
