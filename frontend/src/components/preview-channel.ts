/**
 * <hometube-preview-channel channel-id="...">
 *
 * Parent-side channel preview rendered on /parent/preview/channel/:id.
 * Calls /api/preview/channel/:id (allowlist bypassed) and shows a
 * thumbnail grid of recent uploads. Each thumbnail links to the
 * per-video preview page so the parent can verify content before
...
export class PreviewChannel extends LitElement {
  ...
  override connectedCallback(): void {
    ...
    if (this.channelId) void this.load();
  }
  ...
  override render() {
    ...
    const thumb = pickThumbnail(this.data.thumbnails);
    return html`
      <header>
